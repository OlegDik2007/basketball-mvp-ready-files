...
def ensure_core_tables(cur):
    ...
    """)
    cur.execute("ALTER TABLE games ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'scheduled'")
    cur.execute("ALTER TABLE games ADD COLUMN IF NOT EXISTS home_score INT")
    cur.execute("ALTER TABLE games ADD COLUMN IF NOT EXISTS away_score INT")
    cur.execute("ALTER TABLE games ADD COLUMN IF NOT EXISTS source TEXT DEFAULT 'openclaw'")
    cur.execute("ALTER TABLE games ADD COLUMN IF NOT EXISTS created_at TIMESTAMP DEFAULT NOW()")
    cur.execute("ALTER TABLE games ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW()")
...
<h2>🔥 Value Bets</h2><table><thead><tr><th>Game</th><th>Odds</th><th>Home %</th><th>Edge</th><th>Recommendation</th></tr></thead><tbody id='valueBetsTable'></tbody></table>
<h2>📊 Predictions</h2><table><thead><tr><th>Game</th><th>Home Odds</th><th>Away Odds</th><th>Home %</th><th>Away %</th><th>Edge</th><th>Recommendation</th></tr></thead><tbody id='predictionsTable'></tbody></table>
...
async function loadData(){try{const [games,preds,vals,sigs,bets,perf]=await Promise.all([j('/games'),j('/predictions'),j('/value-bets'),j('/news-signals'),j('/bets'),j('/performance')]);document.getElementById('gamesCount').textContent=games.length;document.getElementById('predictionsCount').textContent=preds.length;document.getElementById('signalsCount').textContent=sigs.length;document.getElementById('openBets').textContent=perf.open_bets;document.getElementById('profit').textContent=money(perf.profit);document.getElementById('profit').className='metric '+cls(perf.profit);document.getElementById('roi').textContent=Number(perf.roi||0).toFixed(1)+'%';document.getElementById('roi').className='metric '+cls(perf.roi);document.getElementById('updated').textContent='Updated: '+new Date().toLocaleString();document.getElementById('betsTable').innerHTML=bets.length?bets.map(b=>`<tr><td>${b.id}</td><td>${b.away_team||'-'} @ ${b.home_team||'-'}</td><td class='bet'>${b.recommendation}</td><td>${b.selected_odds}</td><td class='${cls(b.edge)}'>${edge(b.edge)}</td><td>${money(b.stake_amount)}</td><td>${b.status}</td><td class='${cls(b.result_profit)}'>${money(b.result_profit)}</td><td><button onclick="settle(${b.id},'won')">Won</button> <button onclick="settle(${b.id},'lost')">Lost</button> <button onclick="settle(${b.id},'push')">Push</button></td></tr>`).join(''):'<tr><td colspan=9 class=no>No tracked bets yet</td></tr>';document.getElementById('valueBetsTable').innerHTML=vals.length?vals.map(p=>`<tr><td>${p.away_team||'-'} @ ${p.home_team||'-'}</td><td>${p.home_odds||'-'} / ${p.away_odds||'-'}</td><td>${pct(p.win_prob_home)}</td><td class='${cls(p.edge_home)}'>${edge(p.edge_home)}</td><td class='bet'>${p.recommendation}</td></tr>`).join(''):'<tr><td colspan=5 class=no>No value bets right now</td></tr>';document.getElementById('predictionsTable').innerHTML=preds.map(p=>`<tr><td>${p.away_team||'-'} @ ${p.home_team||'-'}</td><td>${p.home_odds||'-'}</td><td>${p.away_odds||'-'}</td><td>${pct(p.win_prob_home)}</td><td>${pct(p.win_prob_away)}</td><td class='${cls(p.edge_home)}'>${edge(p.edge_home)}</td><td class='${p.recommendation!='NO BET'?'bet':'no'}'>${p.recommendation}</td></tr>`).join('');document.getElementById('signalsTable').innerHTML=sigs.map(s=>`<tr><td>${s.team||'-'}</td><td>${s.player||'-'}</td><td>${s.signal_type||'-'}</td><td>${s.signal_text||'-'}</td><td class='${cls(s.impact_score)}'>${s.impact_score}</td></tr>`).join('')}catch(e){document.getElementById('updated').textContent='Error: '+e.message}}loadData();setInterval(loadData,30000);
</script></body></html>
    """
...
def import_games(payload: GamesImportPayload):
    ...
    for g in payload.games:
        cur.execute("""
            ...
            AND COALESCE(DATE(game_time), CURRENT_DATE)=COALESCE(DATE(%s::timestamp), CURRENT_DATE)
            LIMIT 1
        """, (g.home_team, g.away_team, g.game_time))
        existing = cur.fetchone()
        if existing:
            cur.execute("""
                UPDATE games SET game_time=COALESCE(%s, game_time), home_odds=COALESCE(%s, home_odds), away_odds=COALESCE(%s, away_odds), status=COALESCE(%s,status), source=%s, updated_at=NOW()
                WHERE id=%s
            """, (g.game_time, g.home_odds, g.away_odds, g.status, g.source, existing[0]))
        else:
            cur.execute("""
                INSERT INTO games (home_team, away_team, game_time, home_odds, away_odds, status, source)
                VALUES (%s,%s,%s,%s,%s,%s,%s)
            """, (g.home_team, g.away_team, g.game_time, g.home_odds, g.away_odds, g.status, g.source))
        saved += 1
...
def import_results(payload: ResultsImportPayload):
    ...
    for r in payload.results:
        ...
        if row:
            cur.execute("""
                UPDATE games SET home_score=%s, away_score=%s, status=%s, source=%s, updated_at=NOW()
                WHERE id=%s
            """, (r.home_score, r.away_score, r.status, r.source, row[0]))
        else:
            cur.execute("""
                INSERT INTO games (home_team, away_team, game_time, home_score, away_score, status, source)
                VALUES (%s,%s,%s,%s,%s,%s,%s)
            """, (r.home_team, r.away_team, r.game_time, r.home_score, r.away_score, r.status, r.source))
        saved += 1
    conn.commit(); cur.close(); conn.close()
    return {"status":"ok","saved":saved}
...
def get_bets():
    ...
    cur.execute("""
        ...
        FROM bet_recommendations b LEFT JOIN games g ON g.id=b.game_id ORDER BY b.id DESC LIMIT 100
    """)
    rows = cur.fetchall(); cur.close(); conn.close()
...
def performance():
    conn = db(); cur = conn.cursor(); ensure_tracking_tables(cur); conn.commit()
    cur.execute("""
        SELECT COUNT(*) FILTER (WHERE status='open'), COUNT(*) FILTER (WHERE status!='open'), COUNT(*) FILTER (WHERE status='won'), COUNT(*) FILTER (WHERE status='lost'), COALESCE(SUM(stake_amount) FILTER (WHERE status!='open'),0), COALESCE(SUM(result_profit),0)
